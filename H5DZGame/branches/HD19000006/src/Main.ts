...
import ResList from "./dzgames/configs/rescfg/imgres/res";

export let dzapp:AppPresenter = null;
class Main {
	constructor() {
		//根据IDE设置初始化引擎		
	...
	onConfigLoaded(): void {
		//resource load
		let logoRes:any[] = new Array();
		ResList.Preload.forEach(element => {
			logoRes.push(element);
		});
	 
		ResourceMgr.loadRes(logoRes, Laya.Handler.create(this, this.onResouceLoaded));
	}
	/**
	 * after resource loaded then run presenter.
	 */
	onResouceLoaded():void{
		dzapp = dzapp|| new AppPresenter(); 
		dzapp.startGame();
	}
}
//激活启动类
